...
Usage: python scripts/train_distilbert.py [--imbalance weighted_ce|undersample_ce]
...
logger = get_logger("train_distilbert",
                    os.path.join(config.LOGS_DIR, "train_distilbert.log"))
...
def main():
    ...
    model = DistilBertClassifier(finetune_strategy=args.finetune)

    # Optimizer setup based on strategy
    ...
    model_name = f"distilbert_{args.finetune}_{args.imbalance}"
    ...
    if wandb_enabled:
        ...
        wandb_run = wandb.init(
            ...
            name=run_name,
            tags=["transformer", "distilbert", args.finetune, args.imbalance],
            config={
