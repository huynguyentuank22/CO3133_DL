...
| Family | Best Checkpoint | Macro-F1 | Accuracy |
|---|---|---:|---:|
| BiLSTM | bilstm_weighted_ce | 0.4854 | 0.6524 |
| BiLSTM+Attention | bilstm_attention_weighted_ce | 0.4921 | 0.6356 |
| DistilBERT | distilbert_full_weighted_ce | 0.5421 | 0.6707 |
| BERT-base | bert_llrd_weighted_ce | **0.5646** | **0.6825** |
...
| alpha | accuracy | macro_f1 | weighted_f1 | precision | recall |
|---:|---:|---:|---:|---:|---:|
| 0.3 | 0.6851 | 0.5589 | **0.6929** | **0.5717** | 0.5582 |
| 0.4 | **0.6863** | 0.5599 | 0.6943 | 0.5681 | 0.5612 |
| 0.5 | 0.6834 | **0.5647** | 0.6917 | 0.5660 | **0.5688** |
| 0.6 | 0.6722 | 0.5579 | 0.6817 | 0.5492 | 0.5700 |
| 0.7 | 0.6645 | 0.5498 | 0.6747 | 0.5355 | 0.5694 |

### Single vs Ensemble

| model | accuracy | macro_f1 | weighted_f1 | precision | recall | alpha |
|---|---:|---:|---:|---:|---:|---:|
| BERT-full-undersample | 0.6563 | 0.5490 | 0.6671 | 0.5296 | 0.5790 | - |
| BERT-full-weighted | 0.6772 | 0.5471 | 0.6851 | 0.5632 | 0.5449 | - |
| Ensemble (BERT-full-undersample + BERT-full-weighted, alpha=0.5) | **0.6834** | **0.5647** | **0.6917** | **0.5660** | **0.5688** | 0.5 |

## Robustness (Clean vs Noisy)

| model_name | clean_accuracy | noisy_accuracy | drop_accuracy | clean_macro_f1 | noisy_macro_f1 | drop_macro_f1 |
|---|---:|---:|---:|---:|---:|---:|
| BiLSTM | 0.6524 | 0.6194 | 0.0330 | 0.4854 | 0.4280 | 0.0574 |
| BiLSTM+Attention | 0.6356 | 0.6024 | 0.0333 | 0.4921 | 0.4395 | 0.0525 |
| DistilBERT | 0.6707 | 0.6663 | **0.0044** | 0.5421 | 0.5228 | **0.0193** |
| BERT-base | **0.6825** | **0.6736** | 0.0088 | **0.5646** | **0.5322** | 0.0324 |

## Error Analysis (Best per Family)

| model_family | checkpoint | accuracy | error_rate |
|---|---|---:|---:|
...
1. Mô hình tốt nhất toàn cục hiện tại là bert_llrd_weighted_ce (accuracy 0.6825, macro-F1 0.5646), đứng đầu cả accuracy lẫn macro-F1 trong nhóm mô hình đã chạy.
...
6. Ensemble giữa BERT-full-undersample và BERT-full-weighted cho macro-F1 tốt nhất tại alpha=0.5 (0.5647), nhỉnh hơn mô hình đơn tốt nhất theo macro-F1 một mức rất nhỏ; lợi ích chính là cân bằng precision/recall hơn là tăng mạnh điểm số.
7. Về robustness với nhiễu, DistilBERT ổn định nhất (drop accuracy chỉ 0.0044), còn BERT-base đạt điểm clean cao nhất nhưng giảm macro-F1 nhiều hơn khi thêm nhiễu.
8. Về triển khai thực tế: BiLSTM có tốc độ suy luận nhanh nhất (0.5732 ms/sample), nhanh hơn BERT-base khoảng 11.7x; DistilBERT là điểm cân bằng tốt giữa hiệu năng và chi phí (nhanh gần 2x, nhẹ hơn khoảng 39% so với BERT-base).
